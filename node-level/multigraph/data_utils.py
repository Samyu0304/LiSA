...
def our_forward(source_dataset, model, generators, criterion):
    ...
    loss_local = model(source_dataset, criterion)
    loss_mean += loss_local
    count += 1

    # generate
    for generator in generators:
        #edges, mask = generator(edges)
        generator = torch.clamp(generator, -10, 10)
        mask = F.gumbel_softmax(generator)
        loss_local = model(source_dataset, criterion, mask)
        loss_array.append(loss_local)
        loss_mean += loss_local
        count += 1

    loss_mean = loss_mean / count
    loss_var = 0
    count = 0
    for item in loss_array:
        loss_var += 0.5 * (item - loss_mean) ** 2
        count += 1
